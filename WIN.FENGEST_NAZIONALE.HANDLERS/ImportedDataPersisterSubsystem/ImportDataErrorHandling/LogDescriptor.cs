...
namespace WIN.FENGEST_NAZIONALE.HANDLERS.ImportedDataPersisterSubsystem.ImportDataErrorHandling
{
            ...
            get 
            {
                try
                {
                    return "Importazione: " + DateTime.Now.ToString () + 
                        ". ExportType: " + _export.ExportType.ToString() + 
                     ". Provincia: " + _export.Province.Descrizione + 
                     ". Settore: " + _export.Settore + 
                     ". Periodo: da " + _export.Period.InitialDate.ToShortDateString() +
                     " a " + _export.Period.EndDate.ToShortDateString() + Environment.NewLine;
                }
                catch (Exception)
                {
