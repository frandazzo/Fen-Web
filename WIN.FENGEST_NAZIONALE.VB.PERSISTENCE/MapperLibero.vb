...
#Region "Istruzioni Sql"
    ...
    Protected Overrides Function FindAllStatement() As String
        Return "Select * from lavoratori_liberi"
    End Function

    Protected Overrides Function FindByKeyStatement() As String
        Return "Select * from lavoratori_liberi where Id = @Id"
    End Function

    Protected Overrides Function InsertStatement() As String
        Return "Insert into lavoratori_liberi (CodiceFiscale, Nome, Cognome, NomeCompleto, " _
                & "DataNascita, Sesso, Id_Nazione, NomeNazione, Id_Provincia, NomeProvincia, " _
                & "Id_Comune, NomeComune, Id_Provincia_Residenza, NomeProvinciaResidenza, " _
                & "Id_Comune_Residenza, NomeComuneResidenza, Indirizzo, Cap, Telefono, " _
                & "UltimaModifica, UltimaProvinciaAdAggiornare, CurrentAzienda, IscrittoA, NomeProvinciaFeneal, Id_ProvinciaFeneal, LiberoAl, Ente) " _
                & "values (@cdf, @nom, @cog, @nco, @dan, @ses, @idn, @nmn, @idp, @nmp, @idc, @nmc, " _
                & "@idpr, @nmpr, @idcr, @nmcr, @ind, @cap, @tel, @ulm, @ulp, @curr, @iscra, @nmprf, @idfen, @libal, @ente)"
    End Function
    ...
    Protected Overrides Function DeleteStatement() As String
        Return "Delete from lavoratori_liberi where Id = @Id"
    End Function
...
#Region "Metodi per la ricerca dell'oggetto secondo l'id proposto"
    ...
    Public Overrides Function FindObjectById(ByVal Id As Integer) As AbstractPersistenceObject
        ...
        Return DirectCast(MyBase.FindByKey(New Key(Id)), Libero)


    End Function
    Protected Overloads Overrides Function DoLoad(ByVal Key As Key, ByVal rs As System.Collections.Hashtable) As AbstractPersistenceObject
        Try
            Dim utente As New Libero
            utente.Key = Key

            utente.IscrittoA = IIf(rs.Item("IscrittoA") IsNot Nothing, rs.Item("IscrittoA"), "")
            utente.CurrentAzienda = IIf(rs.Item("CurrentAzienda") IsNot Nothing, rs.Item("CurrentAzienda"), "")
            utente.LiberoAl = rs.Item("LiberoAl")
        ...
        Catch ex As Exception
            Throw New Exception("Impossibile caricare l'oggetto Lavoratore Libero con Id = " & Key.LongValue & ". " & vbCrLf & ex.Message)
        End Try
    End Function
...
#End Region
    ...
    Protected Overrides Sub LoadInsertCommandParameters(ByVal Item As AbstractPersistenceObject, ByVal Cmd As System.Data.IDbCommand)
        Try
            Dim lavoratore As Libero = DirectCast(Item, Libero)
            ...
            If String.IsNullOrEmpty(lavoratore.CurrentAzienda) Then
                param.Value = ""
            Else
                param.Value = lavoratore.CurrentAzienda
            End If
            Cmd.Parameters.Add(param)

            param = Cmd.CreateParameter
            param.ParameterName = "@iscra"
            If String.IsNullOrEmpty(lavoratore.IscrittoA) Then
                param.Value = ""
            Else
                param.Value = lavoratore.IscrittoA
            End If
            Cmd.Parameters.Add(param)

            param = Cmd.CreateParameter
            param.ParameterName = "@nmprf"
            param.Value = lavoratore.Provincia.Descrizione
            Cmd.Parameters.Add(param)


            param = Cmd.CreateParameter
            param.ParameterName = "@idfen"
            param.Value = lavoratore.Provincia.Id
            Cmd.Parameters.Add(param)

            param = Cmd.CreateParameter
            param.ParameterName = "@libal"
            param.Value = lavoratore.LiberoAl
            Cmd.Parameters.Add(param)

            param = Cmd.CreateParameter
            param.ParameterName = "@ente"
            If String.IsNullOrEmpty(lavoratore.Ente) Then
                param.Value = ""
            Else
                param.Value = lavoratore.Ente
            End If
            Cmd.Parameters.Add(param)


        Catch ex As Exception
            Throw New Exception("Impossibile caricare il comando di inserimento dell'oggetto Lavoratore Libero." & vbCrLf & ex.Message)
        End Try
    End Sub
