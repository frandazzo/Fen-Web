...
namespace WIN.FENGEST_NAZIONALE.DOMAIN.ValidationSubsystem
{
    public class ErrorManager
    {
        ...
        //public ExportError CreateExportError(ExportTrace trace, ErrorType errorType, string message, bool containsworkerError, bool containsTraceError)
        //{
        //    DateTime d = DateTime.Now;
        //    //creo il nome del file
        //    string fileName = "ErroreImportazione_" + d.Year + "-" + d.Month + "-" + d.Day + "-" + d.Hour + "-" + d.Minute + "-" + d.Second + ".xml" ;
        ...
        //        ExportTraceSerializzationHandler.Serialize(trace, fileName);
        //    }
        //    catch (Exception ex)
        //    {
        ...
        //    return new ExportError(errorType, d, fileName, containsworkerError, containsTraceError, trace.ExportNumber);
        //}
        ...
        public ExportError CreateExportError(INTEGRATION_ENTITIES.FenealgestImportExport.LiberiTrace trace, ErrorType errorType, string errorMessage, bool containsworkerError, bool containsTraceError)
        {
            DateTime d = DateTime.Now;
            //creo il nome del file
            string file = "";

            if (errorType == ErrorType.LiberiPersistenceError)
                file = "ErroreSottosistemaPersistenzaLiberi_";
            else if (errorType == ErrorType.LiberiDomainError)
                file = "ErroreValidazioneDominioLiberi_";
            else if (errorType == ErrorType.LiberiDTOError)
                file = "ErroreSottosistemaValidazioneLiberi_";
            else
                file = "ErroreImportazioneLiberi_";
            ...
            {
                //creo il file serializzato con l'errore
                LiberiTraceSerializationHandler.Serialize(trace, fileName);
            }
            catch (Exception ex)
            {
                //un qualunque errore in questo contesto mi restituirebbe un altro export error
                ...
                return new ExportError(ErrorType.Unknown, DateTime.Now, "", message, containsworkerError, containsTraceError, 1);
            }
