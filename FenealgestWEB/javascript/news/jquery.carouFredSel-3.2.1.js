...
(function($) {
	JQ.fn.carouFredSel = function(o) {
		...
		this.init = function(o) {
			...
			if (typeof o.scroll == 'number') {
				...
				if (typeof o.scroll == 'string')		o.scroll	= { easing	: o.scroll 	};
				 ...
				 if (typeof o.items  == 'number') 		o.items		= { visible	: o.items 	};
																		...
																		width	: o.items, 
																		height	: o.items	};
			opts = JQ.extend(true, {}, JQ.fn.carouFredSel.defaults, o);
			...
			opts.prev		= JQ.extend({}, opts.scroll, opts.prev);
			opts.next		= JQ.extend({}, opts.scroll, opts.next);
			opts.pagination	= JQ.extend({}, opts.scroll, opts.pagination);

			if (typeof opts.pagination.keys				!= 'boolean')	opts.pagination.keys 			= false;
			if (typeof opts.pagination.anchorBuilder	!= 'function')	opts.pagination.anchorBuilder	= JQ.fn.carouFredSel.pageAnchorBuilder;
		...
		this.build = function() {
			...
			if (opts.usePadding) {
				getItems($cfs).each(function() {
					var m = parseInt(JQ(this).css(opts.dimentions[6]));
					if (isNaN(m)) m = 0;
					JQ(this).data('cfs_origCssMargin', m);
				});
			}
			...
			$cfs.bind('slideTo', function(e, num, dev, org, obj) {
				...
				.bind('insertItem', function(e, itm, num, org, dev) {
					if (typeof itm == 'object' && 
						typeof itm.jquery == 'undefined')	itm = JQ(itm);
					if (typeof itm == 'string') 			itm = JQ(itm);
					if (typeof itm != 'object' || 
		...
		if (JQ(this).parent().is('.caroufredsel_wrapper')) {
			var $wrp = $cfs.parent();
			this.destroy();
		}
		var $wrp			= JQ(this).wrap('<div class="caroufredsel_wrapper" />').parent(),
			opts 			= {},
			totalItems		= getItems($cfs).length,
			firstItem 		= 0,
			autoTimeout		= null,
	...
	JQ.fn.carouFredSel.pageAnchorBuilder = function(nr) {
		return '<a href="#"><span>'+nr+'</span></a>';
	};

	//	private
	function link_anchors($c, se, $cfs) {
		if (typeof $c == 'undefined' || $c.length == 0) $c = JQ('body');
		else if (typeof $c == 'string') $c = JQ($c);
		if (typeof $c != 'object') return false;
		if (typeof se == 'undefined') se = '';
		$c.find('a'+se).each(function() {
			var h = this.hash || '';
			if (h.length > 0 && getItems($cfs).index(JQ(h)) != -1) {
				JQ(this).unbind('click').click(function(e) {
					e.preventDefault();
	...
	function getNaviObject(obj, pagi, auto) {
		...
		if (typeof obj == 'string') {
			...
			if (temp == -1) 			obj = JQ(obj);
			else 						obj = temp;
		}
		if (pagi) {
			if (typeof obj.jquery 		!= 'undefined')	obj = { container: obj };
			if (typeof Object 			== 'boolean')	obj = { keys: obj };
			if (typeof obj.container	== 'string')	obj.container = JQ(obj.container);
			...
			if (typeof obj.button	== 'string')		obj.button = JQ(obj.button);
	...
	function getItemIndex(num, dev, org, firstItem, totalItems, $cfs) {
		if (typeof num == 'string') {
			if (isNaN(num)) num = JQ(num);
			else 			num = parseInt(num);
		}
		if (typeof num == 'object') {
			if (typeof num.jquery == 'undefined') num = JQ(num);
